...
import {FigureLabel, Pony} from "../../../components/mdx/Text";
import ResponsiveEmbed from "../../../components/mdx/ResponsiveEmbed";

**Update 2025:** this blog has since moved from Gatsby to Next.js, mostly to get rid of GraphQL, which is cringe. The sliders survived. The code below is now a historical artifact.

I've been told that my blog is -- and I quote -- aesthetically challenging, unpleasant to look at, and uncanny valley-y. Don't get me wrong, it's ok for an amateurish work, but it's kinda frustrating, because it could have been good with just a little more effort.

Now, faced with such harsh but well worded feedback -- so rare these days! -- my only thought was:
...
So, after cutting ties with her, I set myself to work on a radical design overhaul, wielding a glass of wine and unafraid of using it. 🍷
...
For starters, it seems to be coming directly from 2003, what with that huge six-second holiday video on repeat that doesn't even work on mobile, I suspect for battery reasons.

It has cool micro-animations, and it does the job of giving you a glimpse of the most recent content, but are you feeling compelled to read a post?
...
And I'm not saying this, I'm just quoting your DMs, or at least the parts that I can repeat without blushing, you silly pervs. 🙈
...
I feel like this is exactly what I need! I want you to experience my blog the same way I interact with Netflix, which is to scroll the lists of shows and films for a solid 5 minutes, find nothing of interest, and then resolve to watch for the umpteenth time In Bruges.

<ResponsiveEmbed
    src="https://gifer.com/embed/NRVv"
    ratio="16:9"
/>
...
And, boom, I have all the posts at my fingertips, ready to be grouped as I please. Let's do some of that:
...
Now that we have the CoNtEnT, let's put it to use on an actual page. Based on the blueprint above, this is all there is to it...
...
Let's start with the *pièce de résistance*, the most recent post displayed at the top of the screen for maximum consumption. Here's where it's at:
...
I used to have emojis that appear when you rest the cursor over a button. I want to move away from that, but retain the emojis, so I put them in the bottom right. They really tie the room together.

Speaking of which, we need to talk about the button.

### We need to talk about the button

The button I created for version 1.0 simply won't do with the new style. It's too noisy, too unnecessary in the way it expands when hovering over it. What was I thinking? Why didn't you say anything? And why did I cut ties with those that did?
...
When you hover or tap on a post, more information will be displayed, so that you have no other choice but to read it, unless you want ties to be cut.
...
Was it as exciting to read as it was for me to write? Yes!
...
I have a title on the left, and a subtitle that I choose to render as a tooltip in order to reduce visual overload. On the right, a progress bar reports the number of pages and the one you are currently sitting on.

In order to render the progress bar, I need to know the number of pages. How to calculate it? Easy-peasy:

```js
Math.ceil(posts.length / calculateItemsPerScreen(width))
```
...
When you press either ◀ or ▶ the content will be shifted by ±100% of the screen width. Some easing is applied, as we are not savages.

Now, if you try this on Netflix, you will notice that when the end of content is reached, you loop around to the beginning.

If you have ever implemented a 2D side-scrolling game, you know that the way to do this is to lay down the train tracks while on the train, meaning that once a piece of background reaches the left side of the screen, and it's no longer visible, you can move it to the right side, ready to enter the screen again. Rinse and repeat.

<div style={{ maxWidth: '360px', margin: '2rem auto' }}>
  <ResponsiveEmbed
    src="https://www.tiktok.com/player/v1/7460117650258054443?controls=1&description=0&music_info=0"
    ratio="9:16"
...
Here is the final result. Notice how smooth the sliding effect is! Notice how the items get bigger when you hover on them!
...
I'm sure different browsers and devices will experience bugs. Please report them in the comments, and I will PayPal you a reward. <small>(Might be a bald-faced lie.)</small>

In case you prefer the old page, too bad: it didn't survive the move to Next.js. RIP index-cringe. I'll still be monitoring IP addresses and cutting ties with whoever mourns it.
